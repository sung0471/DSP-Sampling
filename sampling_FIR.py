...
def input_signal(index, sampling_rate, isCos, carrier_frequency, amplitude, phase):
    ...
    for i in range(index):
        ...
        if analog_min_freq>carrier_frequency[i] and carrier_frequency[i]!=0:
...
def getFilterMagnitude(sampling_freq):
    return np.absolute(1/5*(2*np.cos(2*sampling_freq)+2*np.cos(sampling_freq)+1))
...
def firFiltering():
    ...
    for i in range(index):
        firMagni[i]=getFilterMagnitude(a2s_signal[i].frequency*2*np.pi)
        firPhase[i]=getFilterPhase(a2s_signal[i].frequency*2*np.pi)

        fir_Signal[i]=signal(a2s_signal[i])
        fir_Signal[i].amplitude*=firMagni[i]
    ...
    signal_magnitude=[[0.]*index for i in range(2)]
    signal_phase=[[0.]*index for i in range(2)]

    for i in range(index):
        signal_magnitude[0][i]=a2s_signal[i].frequency*2*np.pi
        signal_magnitude[1][i]=firMagni[i]

        signal_phase[0][i]=a2s_signal[i].frequency*2*np.pi
        signal_phase[1][i]=firPhase[i]
...
def recovering(check):
    ...
    print(set_x_Aliasing,set_y_Aliasing,set_x_nonAliasing,set_y_nonAliasing)
...
def printFIRFig():
    ...
    set_y_magnitude = getFilterMagnitude(set_x_magnitude)
    ...
    fig2.stem(signal_magnitude[0], signal_magnitude[1], linefmt='r-', markerfmt='rs', basefmt='k-')
    for x,y in zip(signal_magnitude[0],signal_magnitude[1]):
        fig2.text(x,y,str("%.2e" % y))
    fig2.set_title("Magnitude of the Filter")

    fig5 = fig.add_subplot(2, 3, 5)
    fig5.plot(set_x_phase,set_y_phase)
    fig5.stem(signal_phase[0], signal_phase[1], linefmt='r-', markerfmt='rs', basefmt='k-')
    for x,y in zip(signal_phase[0],signal_phase[1]):
        fig5.text(x,y,str("%.2e" % y))
    fig5.set_title("Phase of the Filter")
    fig5.set_xlabel("Sampling frequency")
...
def printAllfig():
    ...
    for i in range(len(set_x_Aliasing["analog"])):
        set_x_Aliasing_analog2s.append(set_x_Aliasing["analog"][i] * 2 * np.pi / sampling_rate)
        set_x_Aliasing_recover2s.append(set_x_Aliasing["recover"][i] * 2 * np.pi / sampling_rate)
    # print(set_x_Aliasing_analog2s,set_x_Aliasing_recover2s)

    fig5 = fig.add_subplot(3, 4, 10)
    fig5.bar(set_x_nonAliasing_s, set_y_nonAliasing, width=0.5, color='b')
    fig5.bar(set_x_Aliasing_analog2s, set_y_Aliasing["analog"], width=0.5, color='b', alpha=0.4)
    fig5.bar(set_x_Aliasing_recover2s, set_y_Aliasing["recover"], width=0.5, color='r', alpha=0.4)
    fig5.plot(zero_x, zero_y, 'k-')
    ...
    fig7 = fig.add_subplot(3, 4, 3)
    fig7.plot(set_x_magnitude,set_y_magnitude)
    fig7.stem(signal_magnitude[0], signal_magnitude[1], linefmt='r-', markerfmt='rs', basefmt='k-')
    for x,y in zip(signal_magnitude[0],signal_magnitude[1]):
        fig7.text(x,y,str("%.2e" % y))
    fig7.set_title("Magnitude of the Filter")

    fig8 = fig.add_subplot(3, 4, 11)
    fig8.plot(set_x_phase,set_y_phase)
    fig8.stem(signal_phase[0], signal_phase[1], linefmt='r-', markerfmt='rs', basefmt='k-')
    for x,y in zip(signal_phase[0],signal_phase[1]):
        fig8.text(x,y,str("%.2e" % y))
    fig8.set_title("Phase of the Filter")
    fig8.set_xlabel("Sampling frequency")
